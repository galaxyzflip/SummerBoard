...
<body onload="selectedOptioncheck()">
	...
	<div class="wrapper">
		...
		<div class="navBar">
			...
			<form action="list.do" method="get">
				...
				<select id="type" name="type">
					...
					value="<c:if test="${keyword != null}">${keyword }</c:if><c:if test="${keyword == null}"></c:if>" />
				...
				<c:forEach var="board" items="${boardList }">
					<tr>
						...
						<td align="left" class="subject"><c:if
							...
							</c:if> <a href="view.do?idx=${board.idx }">${board.subject }</a>
						</td>
		...
		<br> ${pageHtml } <br>
		<br> <input type="button" value="목록" class="writeBt" onclick="moveAction(2)" />
